...
write-host @'
***************** 
Yazar: @xcientist Dikkat Fakirler için yapilmistir zenginler kullanamaz kullansada sayilmaz.
***************** 
'@
...
if (Get-AppxPackage -Name SpotifyAB.SpotifyMusic) {
  Write-Host @'
Microsoft Store version bulundu bunu desteklemiyorumki kanka.
'@`n
  $ch = Read-Host -Prompt "Windows Store versiyonu silelim orjinali yükleyelim YES de (Y/N) "
  if ($ch -eq 'y'){
     Write-Host @'
...
Write-Host 'Kanka en son versiyon kirek indiriyorum az bekle (chrome_elf.zip)...'`n
...
if (-not $spotifyInstalled) {
  Write-Host @'
Spotify yuklu degilki kanka .
Senin icin indireyim, biraz bekle...
'@
  try {
    $webClient.DownloadFile(
      # Remote file URL
...
Write-Host 'Zıbam Zıbam bitti gitti artık reklamsiz dinleyebilirsin.'
